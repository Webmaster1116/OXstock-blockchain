<?php
...

	{
...

		{
			...
			$status = fetchqry('*',TB_SUB_CATEGORIES,array("id="=>$_REQUEST['id']));;

			if($status['status']=='0')

			{ 

				updateqry(array("status"=>1),array("id="=>$_REQUEST['id']),TB_SUB_CATEGORIES);
			...
			{ 
				...
				updateqry(array("status"=>0),array("id="=>$_REQUEST['id']),TB_SUB_CATEGORIES);
...

		{
...

			{
				...
				$chkqry = "SELECT * FROM `".TB_SUB_CATEGORIES."` WHERE url = '".$value."' ";
