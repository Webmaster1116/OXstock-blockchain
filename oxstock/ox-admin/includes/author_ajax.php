<?php
...

	{
...

		{
			...
			$status = fetchqry('*',TB_AUTHOR,array("id="=>$_REQUEST['id']));;

			if($status['status']=='0')

			{ 

				updateqry(array("status"=>1),array("id="=>$_REQUEST['id']),TB_AUTHOR);
			...
			{ 
				...
				updateqry(array("status"=>0),array("id="=>$_REQUEST['id']),TB_AUTHOR);
