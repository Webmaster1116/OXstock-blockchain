...
if (($permission['add'] && !$_REQUEST['id']) || ($permission['edit'] && $_REQUEST['id'])) {
    ...
    if ($_REQUEST['id']) {
        ...
        $qry = "select `invoice`.`id`, `invoice`.`status`, `invoice`.`item`, `invoice`.`sub_total`, `invoice`.`comments`, `invoice`.`image`, `invoice`.`invoice_date`, `invoice`.`due_date`, `invoice`.`share_status`, `invoice`.`created_at`";
        ...
        $qry .= " where 1 ";
        $qry .= " AND `invoice`.`id`=" . $_REQUEST['id'];
        if ($swords) {
            $qry .= "and 1 ";
            $paginationback .= "&s=" . $swords;
        }
        /** For Group By * */
        $qry .= " group by `invoice`.`id` ";
        /** For Ordering * */
        $qry .= " order by `invoice`.`id` DESC";
                                                        ...
                                                        <div style="padding: 20px;">
                                                             <?php if ($display == 1) { ?>                                                                       
                                                                ...
                                                                <div class="row" style="margin-top: 10px;">
                                                                    <div class="col-sm-2 text-right ">Bill To:</div> 
                                                                    <div class="col-sm-4 text-left"><?php echo $row['client_name']; ?></div> 
                                                                    <div class="col-sm-2 text-right">Invoice No:</div> 
                                                                    <div class="col-sm-4 text-left"><?php echo $row['id']; ?></div>                                                                             
                                                                </div>
                                                                <div class="row" style="margin-top: 10px;">
                                                                    <div class="col-sm-2 text-right">Phone:</div> 
                                                                    <div class="col-sm-4 text-left"><?php echo $row['phone']; ?></div> 
                                                                    <div class="col-sm-2 text-right">Invoice Date:</div> 
                                                                    <div class="col-sm-4 text-left"><?php echo date('F j, Y', strtotime($row['invoice_date'])); ?></div>                                                                             
                                                                </div>
                                                                <div class="row" style="margin-top: 10px;">
                                                                    <div class="col-sm-2 text-right">Address:</div> 
                                                                    <div class="col-sm-4 text-left"><?php echo $row['billing_address']; ?></div>
                                                                    <div class="col-sm-2 text-right">Due Date:</div> 
                                                                    <div class="col-sm-4 text-left"><?php echo date('F j, Y', strtotime($row['due_date'])); ?></div> 
                                                                </div>
                                                                <div class="row" style="margin-top: 10px;"> 
