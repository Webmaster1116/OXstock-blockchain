...
class PdoWrapper extends PDO {
    ...
    public function error( $msg ) {
        ...
        if ( $this->log ) {
            ...
            $data = '';
            $data .= "<div style='color:#990099; border:1px solid #777; padding:2px; background-color: #E5E5E5;'>";
            $data .= " Executed Query -> <span style='color:#008000;'> ";
            $data .= $this->helper()->formatSQL( $this->interpolateQuery() );
            $data .= "</span></div>".$this->helper()->errorBox($msg,false);
            echo $data;die;
            //$this->helper()->error();
            ...
            $data .= $this->helper()->formatSQL( $this->interpolateQuery() );
            $data .= "</span></div>".$this->helper()->errorBox($msg,false);
            $data .='<br/>Module Name - '.$module;
            sendEmailAddress('kishan.chauhan@ncrypted.com','SQL Error Log',$data);*/
            //new code mail
        }
    }
    /**
